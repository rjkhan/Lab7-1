...
We see that the best RMSE value usually comes from the linear regression model with forward selection where there are between two and 12 predictors. This RMSE value is usually a little better than the one from the regular linear regression model. We now try to train our own custom made ridge regression model with a particular set.seed().
...
Preprocessing of data is actually done inside the ridgeregr() function. It seems that based on mimimal RMSE value, parameter lambda wants to be around 40. This value changes a little each time we knit the .rmd document, despite the set.seed(). Now we evaluate all three models on the test dataset.
...
plot(lm_testres,main="Residuals for the linear regression predictions over test data",
...
plot(lmfor_testres,main=c("Residuals for the linear regression with"," forward selection predictions over test data"),
     ylab="residual",xlab="test data observation #")
...
plot(ridge_testres,main="Residuals for the ridge regression predictions over test data",
     ylab="residual",xlab="test data observation #")
paste0("RSS = ",sum(ridge_testres^2))
#The final models can be found under (train obj)$finalModel 
```

It seems the ridge regression is a better fit than linear regression or linear regression with forward selection based on RSS.
